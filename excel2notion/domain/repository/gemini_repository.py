...
class GeminiRepository:
    """Gemini API 접근을 담당하는 Repository"""
    
    def __init__(self):
        api_key = get_gemini_api_key()
        if not api_key:
            raise ConfigurationException(detail="Gemini API 키가 설정되지 않았습니다.")
    ...
    def classify_to_structured_data(self, text: str) -> Dict[str, any]:
        ...
        prompt = f"""
다음 텍스트를 분석하여 식당 정보를 추출하세요. 다음 형식의 JSON으로 응답하세요:
{{
    "시그니처메뉴": "시그니처 메뉴 이름",
...
"""
        try:
            ...
            logger.error(f"Failed to classify text with Gemini: {str(e)}")
            raise ValueError(f"Gemini 분류 실패: {str(e)}")
...
추천 근거를 {similarity_percent}% 반영하고,

{100-similarity_percent}%는 새로운 내용을 작성하세요.
...
'''
    ...
    def extract_recommended_drink(self, top1_data: Dict[str, any]) -> str:
        """추천 식당의 주류 추출"""
        prompt = f"""
다음 식당 정보를 바탕으로 이 식당에 어울리는 주류를 추천해주세요.
...
"""
    ...
    def generate_top1_recommendation_reason(self, top1_data: Dict[str, any]) -> str:
        ...
        prompt = f"""
다음 식당 정보를 바탕으로 이 식당을 추천하는 이유를 작성해주세요.
